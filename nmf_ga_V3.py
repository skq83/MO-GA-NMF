# -*- coding: utf-8 -*-
"""
Created on Thu Apr 22 03:56:24 2021
...
if __name__ == '__main__':
    ...
    Third_GA_Run_Best_pop=Best_pop

    # open a pickle file
    filename = 'GA_v3_pickle.pk'
    
    with open(filename, 'wb') as fi:
        # dump your data into the file
        pickle.dump(Third_GA_Run_Best_pop[1], fi)
    
    #Changing the default python working directory 
    import os
    os. getcwd()
    os.chdir('D:\BCU University\Course\CMP7213-A-S2-20201 - Dr Atif Azad\Assessment\Assessment 1.2\GA')
    
    
    # Reading the three pickle files that includes the results of the three experiements 
    filename = 'GA_v1_pickle.pk'
    with open(filename, 'rb') as fi:
        First_GA_Run_Best = pickle.load(fi)
    
    filename = 'GA_v2_pickle.pk'
    with open(filename, 'rb') as fi:
        Second_GA_Run_Best = pickle.load(fi)
    
   
    
    filename = 'GA_v3_pickle.pk'
    with open(filename, 'rb') as fi:
        Third_GA_Run_Best = pickle.load(fi)
    
    # Standerd error calculation for each variation:
        
    First_std_err = np.std(First_GA_Run_Best,ddof=1) / np.sqrt(len(First_GA_Run_Best))       
    Second_std_err = np.std(Second_GA_Run_Best,ddof=1) / np.sqrt(len(Second_GA_Run_Best))
    Third_std_err = np.std(Third_GA_Run_Best,ddof=1) / np.sqrt(len(Third_GA_Run_Best))
    
    
    # Error Plot / Figure 3 in the report.
    plt.figure(figsize=(20,10))
    generation = [x for x in range(len(Third_GA_Run_Best))]
    fig, ax = plt.subplots()
    ax.errorbar(generation, First_GA_Run_Best,yerr = First_std_err, solid_capstyle='projecting', capsize=2,label='1st GA Run, Pop size:50, MR:0.1')
    ax.errorbar(generation, Second_GA_Run_Best,yerr = Second_std_err, solid_capstyle='projecting', capsize=2,label='2nd GA Run, PS:100, MR:0.1')
    ax.errorbar(generation, Third_GA_Run_Best,yerr = Third_std_err, solid_capstyle='projecting', capsize=2,label='3rd GA Run, Pop size:125, MR:0.1')
    plt.xlabel('Generation')
    plt.ylabel('Best score (Fitness)')
    plt.legend()
